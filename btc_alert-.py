...
def notify(signal, prob, price, current_candle_time):
    # Calcula o horário da vela de ENTRADA (próxima vela = +15 min)
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Alerta enviado: {direction} | Entrada: {entry_str} | Prob: {prob_pct}%")
...
def check_signal():
    ...
    try:
        ...
        if signal and bar_id != last_signal_bar:
            ...
            notify(signal, prob, row["close"], row["timestamp"])
        else:
...
if __name__ == "__main__":
    print("=" * 50)
    print("  BTC M15 — Monitor de Sinais ML")
